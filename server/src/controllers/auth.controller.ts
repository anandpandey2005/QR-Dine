...
export const authRegistration = async (
    req: Request<{}, {}, IAuthRegistration>, 
    res: Response
) => {
    let { 
        fullName, gmail, otp, phone, password, 
        cnfrmPassword, role = 'customer', userId 
    } = req.body || {};
    ...
    if (password !== cnfrmPassword) {
        return res.status(400).json({ data: null, success: false, message: "Password doesn't match confirm password" });
    }
    ...
    try {
        ...
        if (is_gmail_phone_exists) {
            return res.status(409).json({ success: false, message: "User with this gmail or phone already exists." });
        }
        ...
        switch (role) {
            case 'customer': roleModel = 'CustomerProfile'; break;
            case 'manager': roleModel = 'ManagerProfile'; break;
            case 'groundstaff': roleModel = 'GroundStaffProfile'; break;
            case 'chef': roleModel = 'ChefProfile'; break;
            default: 
